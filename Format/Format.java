...
public class Format {
    ...
    public void encryption(String fileName) throws IOException {
        ...
        int word = 0;
        while (word < wordNum) {
            char[] L = new char[Text[word].length()];
            Text[word].getChars(0, Text[word].length(), L, 0);
            for (int i = 0; i < L.length; i++) {
                Integer t = new Integer(L[i]);
                writer.write(t + "");
                writer.write(" ");
            }
            writer.write("_ ");
    ...
    public void decryction(String fileName1, String fileName2) throws IOException {
        BufferedReader reader = new BufferedReader(new FileReader(fileName1));
        FileWriter writer = new FileWriter(fileName2, false);
        String s;
        s = reader.readLine();
        StringTokenizer st = new StringTokenizer(s);
        wordNum = 0;
        while (st.hasMoreTokens()) {
            Text[wordNum] = st.nextToken("_");
            wordNum++;
        }
        //для каждого слова
        for (int i = 0; i < wordNum; i++) {
            ...
            while(stringTokenizer.hasMoreTokens()){
                Integer tmp = Integer.parseInt(stringTokenizer.nextToken(" "));
                writer.write(tmp);
            }
            writer.write(" ");
        }
